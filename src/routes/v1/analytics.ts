...
router.get(
  ...
  async (req, res) => {
    try {
      const reservationMetricRepository =
        AppDataSource.getRepository(ReservationMetric);
      ...
      returnErrorResponse(res, 500, "Database error");
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      const results = await reservationMetricRepository
        ...
        .addSelect("COUNT(*)", "count")
        .where("metric.action IN (:...actions)", {
